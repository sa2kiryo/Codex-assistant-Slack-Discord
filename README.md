...
このシステムは単なるチャットBotではなく、**共感型AI**として設計されている。

### 3つの設計思想

1. **キャラクター人格** — AIに名前・性格・口調・動機を与え、一貫した人格として振る舞わせる
2. **記憶と関係構築** — ユーザーごとの対話履歴を蓄積し、過去のやり取りを踏まえた応答を行う
3. **共感ファースト** — 「まず共感、次に解決策」を行動原則とし、感情を検知して自発的に声をかける
...
4. OAuth2 URL Generator で `bot` スコープ + 必要な権限を選択してサーバーに招待
